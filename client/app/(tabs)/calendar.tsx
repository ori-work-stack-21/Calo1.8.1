...
export default function CalendarScreen() {
  ...
  const handleAddEvent = async () => {
    if (!newEventTitle.trim() || !selectedDate) {
      Alert.alert(t("calendar.events.error"), t("calendar.events.titleRequired"));
      return;
    }
        ...
        { text: t("common.cancel"), style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              hideEventModal();
              Alert.alert(t("common.success"), t("calendar.events.deleteSuccess"));
              await loadCalendarData();
            } catch (error) {
              Alert.alert(t("calendar.events.error"), t("calendar.events.deleteError"));
            }
          },
        },
      ]
  ...
  const renderCalendarDay = (dayInfo: any, index: number) => {
    ...
    const progressColor = data ? getProgressColor(data.calories_actual, data.calories_goal) : "#6b7280";
              ...
              <>
                {editingEvent ? (
                  ...
                  <>
                      ...
                      <View style={styles.eventMeta}>
                        <View
                          style={[
                            ...
                            {
                              backgroundColor:
                                EVENT_TYPES.find((t) => t.key === selectedEvent.type)
                                  ?.color + "20" || "#6b728020",
                            },
                          ]}
                        >
                          ...
                          >
                            {EVENT_TYPES.find((t) => t.key === selectedEvent.type)?.label ||
                              selectedEvent.type}
                          </Text>
                        </View>
                    ...
                    {selectedDayData?.events && selectedDayData.events.length > 1 && (
                      <View style={styles.otherEventsSection}>
                        <Text style={styles.sectionTitle}>Other Events Today</Text>
                        {selectedDayData.events
                          ...
                          .map((event) => (
                            ...
                            >
                              <Text style={styles.otherEventTitle}>{event.title}</Text>
                              <Text style={styles.otherEventType}>{event.type}</Text>
                            </TouchableOpacity>
                          ))}
                      </View>
                    )}
      ...
      >
        ...
        <View style={styles.calendarContainer}>
          ...
          <View style={styles.calendarGrid}>
            {getDaysInMonth.map((dayInfo, index) => renderCalendarDay(dayInfo, index))}
          </View>
        </View>
          ...
          <View style={styles.statsSection}>
            ...
            <View style={styles.statsContainer}>
              <View style={styles.statItem}>
                ...
                <Text style={styles.statNumber}>{statistics.totalGoalDays}</Text>
                <Text style={styles.statText}>Goal Days</Text>
              </View>
              <View style={styles.statItem}>
                <Target size={24} color="#f59e0b" />
                <Text style={styles.statNumber}>{statistics.monthlyProgress}%</Text>
                    ...
                    >
                      <View
                        style={[
                          ...
                          {
                            backgroundColor:
                              EVENT_TYPES.find((t) => t.key === event.type)?.color ||
                              "#6b7280",
                          },
                        ]}
                      />
